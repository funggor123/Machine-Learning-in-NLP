...
def read_data(file_name, vocab=None):
    ...
    return df['id'], df['label'], data_matrix, vocab, vocab_dict
...
def train_NB(data_label, data_matrix):
    ...
    for column in range(K):
        P_xy[:, column] = normalize(P_xy[:, column], 1)
    P_y = P_y/N
    return P_y, P_xy


def predict_NB(data_matrix, P_y, P_xy, vocab_dict):
    '''
    :param data_matrix: [N(document_number), V(Vocabulary number)], type:  scipy.sparse.csr_matrix
    ...
    for row, col, data in zip(ind_set.row, ind_set.col, ind_set.data):
        for k in range(K):
            print((list(vocab_dict.keys())[list(vocab_dict.values()).index(col)]), ", prob : ", P_xy[col][k], " num :",
                  data, " " , k)
            scores[row, k] += np.log(data * P_xy[col][k])
    # Compute Log(P( Y))
    for col in range(K):
...
if __name__ == '__main__':
    train_id_list, train_data_label, train_data_matrix, vocab, vocab_dict = read_data("data/train.csv")
    print("Vocabulary Size:", len(vocab))
    print("Bag of Words:", vocab_dict)
    print("Training Set Size:", len(train_id_list))
    test_id_list, _, test_data_matrix, _ ,vocab_dict = read_data("data/test.csv", vocab)
    print("Test Set Size:", len(test_id_list))

    P_y, P_xy = train_NB(train_data_label, train_data_matrix)
    test_data_pre = predict_NB(test_data_matrix, P_y, P_xy, vocab_dict)
